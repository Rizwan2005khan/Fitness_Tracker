...
const Onboarding = () => {
  ...
  const updatedField = (field: keyof ProfileFormData, value: string | number) => {
      ...
      <div className="onboarding-container">
        ...
        <div className="px-6 mb-8 onboarding-wrapper">
          <div className="flex gap-2 max-w-2xl">
            {[1,2,3].map((s) => (
              <div key={s} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${s <= step ? "bg-emerald-500" : "bg-slate-200 dark:bg-slate-200"}`} />
            ))}
          </div>
        ...
        <div className="flex-1 px-6 onboarding-wrapper">
          {step === 1 && (
            <div className="space-y-6">
              ...
              <Input label="Age" type="number" className="max-w-2xl" value={formData.age} onChange={(v) => updatedField('age', v)} placeholder="Enter your age" min={13} max={120} required/>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6 onboarding-wrapper">
              ...
              <Input label="Weight (kg)" type="number"  value={formData.weight} onChange={(v) => updatedField('weight', v)} placeholder="Enter your weight" min={20} max={300} required/>

              <Input label="Height (cm) - Optional" type="number"  value={formData.height} onChange={(v) => updatedField('height', v)} placeholder="Enter your height" min={100} max={250} required/>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 onboarding-wrapper">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">We'll tailor your your experience</p>
                </div>
              </div>
              ...
              <div className="space-y-8 max-w-lg">
                ...
                <div className="space-y-6">
                  <Slider label="Daily Calorie Intake" min={120} max={4000} step={50} value={formData.dailyCalorieIntake} onChange={(v) => updatedField('dailyCalorieIntake', v)} unit="kcal" infoText="The total calories you plan to consume each day." />

                  <Slider label="Daily Calorie Burn" min={100} max={2000} step={50} value={formData.dailyCalorieBurn} onChange={(v) => updatedField('dailyCalorieBurn', v)} unit="kcal" infoText="The total calories you aim to burn through exercise and activity each day." />
